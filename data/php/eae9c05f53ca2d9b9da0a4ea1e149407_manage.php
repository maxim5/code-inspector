...
class nggManageGallery {
	...
	function update_pictures() {
		 ...
		 * 	????????????????????????????
		 */
		do_action ( 'bp_alert_gallery_updated', $this->gid );
	...
	function get_editable_user_ids( $user_id, $exclude_zeros = true ) {
		...
		//lvjiyun
		$query = "SELECT user_id FROM $wpdb->usermeta WHERE meta_key='wp_capabilities' and (meta_value like '%\"editor\"%' or meta_value like '%\"administrator\"%')";
