...
// *** show picture list
    ...
    if ($is_search) {
		...
		//lvjiyun
		$style_mt = $_POST['style_mt'];
		if(empty($style_mt)){
			$style_mt = $_GET['style_mt'];
		}
		if(!empty($style_mt)){
			$main_style_type_en_cn = array('hair'=>'??', 'beauty'=>'??', 'clothes'=>'??', 'watches'=>'??', 'handbags'=>'??', 'shoes'=>'?', 'sunglasses'=>'???', 'jewelry'=>'??', 'hats'=>'??', 'accessories'=>'??', 'perfume'=>'??');
			$style_mt = $main_style_type_en_cn[$style_mt];
		}
		//lvjiyun
		
		// get picture values
		//$picturelist = $nggdb->get_gallery($act_gid, $ngg->options['galSort'], $ngg->options['galSortDir'], false, 50, $start );
		$picturelist = $nggdb->get_gallery($act_gid, $ngg->options['galSort'], $ngg->options['galSortDir'], false, 50, $start, false, $style_mt );
...
<script type="text/javascript" src="/wp-content/plugins/nextgen-gallery-custom-fields/ajax.js"></script>
<script type="text/javascript" src="/wp-content/plugins/nextgen-gallery-custom-fields/tags.js"></script>
<div class="wrap">
...
<div class="tablenav ngg-tablenav">
	...
	<!-- lvjiyun begin -->
	&nbsp;&nbsp;&nbsp;&nbsp;??????<select id="style_mt" name="style_mt">
	<option value="">---???---</option>
	<?php 
	$style_mt = $_POST['style_mt'];
	if(empty($style_mt)){
	...
	$main_style_type_cn_en = array('??'=>'hair', '??'=>'beauty', '??'=>'clothes', '??'=>'watches', '??'=>'handbags', '?'=>'shoes', '???'=>'sunglasses', '??'=>'jewelry', '??'=>'hats', '??'=>'accessories', '??'=>'perfume');
	foreach($main_style_type_cn_en as $mt_cn => $mt_en){
		echo '<option value="'.$mt_en.'"';
		if($style_mt == $mt_en){
			echo ' selected';
		}
		echo '>'.$mt_cn.'</option>';
	...
	?>
	</select>&nbsp;<input type="button" name="search_mt" value="??" onclick="var objSelect=document.getElementById('style_mt');window.location.href='/wp-admin/admin.php?page=nggallery-manage-gallery&mode=edit&gid=<?php echo $act_gid;?>&style_mt='+objSelect.options[objSelect.selectedIndex].value;"/>
	<!-- lvjiyun end -->
	</div>
</div>
			...
			foreach($image_columns as $image_column_key => $column_display_name) {
				...
				switch ($image_column_key) {
					...
					case 'tags' :
						...
						//coding start,js was included at line 232-233,by pengbo.
						$gid=$wpdb->get_var("select galleryid from wp_ngg_pictures where pid='{$wpdb->escape($pid)}'");
						$is_gallery_type_cn=$wpdb->get_row("select * from wp_nggcf_fields_link where gid='".$gid."'and field_id='6'",ARRAY_A);
						...
						$type_cn=$wpdb->get_var("select field_value from wp_nggcf_field_values where fid = '6' AND pid = '$pid' ");
						$pos = mb_strpos($type_cn, "-", 0);
                        $mt = mb_substr($type_cn, 0, $pos);
                        $st = mb_substr($type_cn, $pos + 1, mb_strlen($type_cn) - $pos - 1);
                        $sql = "(select distinct tag_name from tb_picture_tags a, tb_stylebook_pictures b where a.pid=b.pid and b.p_main_type='".$mt."' and b.p_sub_type='".$st."') union (select distinct tag_name from tb_standard_styletype_pictags where main_type='".$mt."' and sub_type='".$st."')";
                        $tags = $wpdb->get_col($sql);
						?>
						<td <?php echo $attributes ?>>
						<select name="tags[<?php echo $pid;?>]" id="tags[<?php echo $pid;?>]"  style="width:150%;">
						<option value=""></option>
						<?php
						 foreach($tags as $tag){
						 ?>
						 <option value="<?php echo $tag;?>"<?php echo $tag == $picture->tags ? " selected" : "" ;?>><?php echo $tag;?></option>
						<?php
						}
						?>
						</select></td>
						...
						//coding end,any questions skype to pengbo.						
					break;
					default : 
						?>
