...
struct varobj_root
  {
    ...
    /* If true, "update" always recomputes the frame & valid block
       using the currently selected frame. */
    boolean use_selected_frame;

    /* If true, "update" always recomputes the var exp and car value.
       This is set during purging of symbol info. */
    boolean recompute;
  };
...
static boolean rerun_set = false;
...
void
reset_rerun (void* arg)
{
  struct varobj_root *croot = rootlist;
  int mycount = rootcount;
  while ((croot != NULL) && (mycount > 0))
    {
      croot->rootvar->root->recompute = true;
      mycount--;
      croot = croot->next;
    }
  rerun_set = false;
...
{
  struct varobj *var;
  struct frame_info *fi, *old_fi = 0; /* initialize for compiler warning */
  struct block *block;
  ...
  if (expression != NULL)
	...
	fi = find_frame_addr_in_frame_chain (frame, NULL);

      /* frame = -2 means always use selected frame */
      if (type == USE_SELECTED_FRAME)
	var->root->use_selected_frame = true;
      ...
      if (var->root->exp->elts[0].opcode == OP_TYPE)
	{
      ...
      var->name = savestring (expression, (int) strlen (expression));
	...
	select_frame (old_fi, -1);
      ...
      if (!rerun_set)
	{
          make_objfile_purge_cleanup (reset_rerun, (void *) NULL);
	  rerun_set = true;
	}
    }
  ...
  if ((var != NULL) && (objname != NULL))
    {
      var->obj_name = savestring (objname, (int) strlen (objname));
...
varobj_get_handle (char *objname)
{
  ...
  for (chp = objname; *chp; chp++)
    {
      index = (index + (i++ * (unsigned int)(unsigned char) *chp)) % VAROBJ_TABLE_SIZE;
    }
...
{
  int changed = 0, old_selected_level, diff_levels;
  int type_changed;
  int i;
  int vleft;
  int error2;
  struct varobj *v;
  struct varobj **cv;
  struct varobj **templist = 0; /* initialize for compiler warning */
  value_ptr new;
  ...
  /* Save the selected stack frame, since we will need to change it
   ...
   old_fi = selected_frame; 
   /* Save the selected_frame_level instead of selected_frame which might be pointing
      to already free`d heap after flushing cached frames */
   old_selected_level = selected_frame_level;
  ...
  *cv = vpop (&result);
  ...
  /* The selected_frame may be pointing to invalid heap after call to flush_cached_frames ().
     To restore selected frame, make use of selected_frame_level */
  diff_levels = selected_frame_level - old_selected_level;
  
  /* select_frame (old_fi, -1); */
  if (diff_levels == 0) 
    select_frame (get_current_frame (), 0);
  else
    select_frame (find_relative_frame (get_current_frame (), &diff_levels), old_selected_level);
...
static int
delete_variable (struct cpstack **resultp, struct varobj *var, int only_children_p)
{
  int delcount = 0;
...
new_root_variable (void)
{
  ...
  var->root->frame = (CORE_ADDR)(long) -1;
  var->root->use_selected_frame = false;
  var->root->recompute = false;
...
/* What is the value_ptr of the root variable VAR? 
   ...
   use_selected_frame = true variable changes.  On input,
...
value_of_root (struct varobj ** var_handle, int *type_changed)
{
  ...
  /* If recompute is true, redo the variable creation as the old stuff
     is all invalid. */
  if (var->root->recompute)
    {
	...
	tmp_var = varobj_create (NULL, var->name, var->root->frame,
                               USE_SPECIFIED_FRAME);
      if (tmp_var == NULL)
        {
          return NULL;
      ...
      tmp_var->obj_name =
                savestring (var->obj_name, (int) strlen (var->obj_name));
      uninstall_variable (var);
...
{
  return savestring (parent->name, (int) strlen (parent->name));
}
...
c_name_of_child (struct varobj *parent, int index)
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_UNION:
      ...
      name = savestring (string, (int) strlen (string));
      break;
...
c_value_of_root (struct varobj **var_handle)
{
  ...
  if (var->root->valid_block == NULL)
  ...
  else
    {
      ...
      fi = find_frame_addr_in_frame_chain (var->root->frame, NULL);
  ...
  switch (TYPE_CODE (type))
	...
	if (VALUE_LAZY (val))
	  gdb_value_fetch_lazy (val);
        format[0] = format_code[var->format];
...
cplus_name_of_child (struct varobj *parent, int index)
{
  ...
  if (name == NULL)
    ...
    {
      if (name != NULL)
	name = savestring (name, (int) strlen (name));
    }
