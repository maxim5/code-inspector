...
-- @param mode Array of modes this user has.
function _add_user(self, user, mode)
	if user then
		self._members[user] = {mode = mode or {}, nick = user};
	end
end
-- }}}
...
function _change_status(self, user, on, mode)
	if on then
		self._members[user].mode[irc.usermodes[mode]] = true;
	else
		self._members[user].mode[irc.usermodes[mode]] = nil;
	end
end
-- }}}
...
function _change_nick(self, old_nick, new_nick)
    for member in self:each_member() do
        if member.nick == old_nick then
            local mode = self._members[old_nick].mode
            self._members[old_nick] = nil
            self._members[new_nick] = {mode = mode, nick = new_nick};
            break
        end
    end
end
...
function each_op(self)
    return function(state, arg)
               return misc._value_iter(state, arg,
                                       function(v)
                                           return (v.mode["~"] == true) or (v.mode["&"] == true) or (v.mode["@"] == true)
                                       end)
           end,
           self._members,
           nil
...
function each_user(self)
    return function(state, arg)
               return misc._value_iter(state, arg,
                                       function(v)
                                           return (misc._array_count(v.mode) == 0)
                                       end)
           end,
           self._members,
           nil
