...
function make_channel(parent, stream, config)
    ...
    -- statp
    stat_packet = {}
    if stream.config.dvb then
        stat_packet.source = "dvb"
        if stream.config.dvb.tp then
            stat_packet.tp = stream.config.dvb.tp
        else
            stat_packet.tp = "DVB-" .. stream.config.dvb.type
        end
    elseif stream.config.udp then
        stat_packet.source = "udp"
        stat_packet.addr = stream.config.udp.addr
        stat_packet.port = stream.config.udp.port
    elseif stream.config.file then
        stat_packet.source = "file"
        stat_packet.filename = stream.config.file.filename
    else
        stat_packet.source = "unknown"
    end
    ch.statp = module_attach(ch, statp(stat_packet))
    tail = ch.statp
