...
-- initialise logcat.
local logcat = base.logcat;
local Sakuya = base.Sakuya;

-- user modes and symbols that prefix nicks in channels
usermodes_symbols = {};
usermodes = {};

-- start adding user modes to the table.
usermodes["q"], usermodes_symbols["~"] = "~", "q";
usermodes["a"], usermodes_symbols["&"] = "&", "a";
usermodes["o"], usermodes_symbols["@"] = "@", "o";
usermodes["h"], usermodes_symbols["%%"] = "%%", "h";
usermodes["v"], usermodes_symbols["+"] = "+", "v";
...
-- message to be sent on ctcp finger.
FINGERMSG = "Don't finger me, pervert!";
-- sent on CTCP version
VERSION = "LuaIRC";
...
function handlers.on_nick(from, new_nick)
    ...
    callback("nick_change", from, new_nick)
end
-- }}}
...
function handlers.on_part(from, chan, part_msg)
    ...
    if serverinfo.channels[chan].join_complete then
		...
		if NICK == from then
			serverinfo.channels[chan] = nil;
			logcat(Sakuya.Logcat.RawIRC, "Parted from "..chan.." successfully.");
		end
    end
end
-- }}}
...
function handlers.on_mode(from, to, mode_string, ...)
    ...
    if to:sub(1, 1) == "#" then
        ...
        for i = 1, mode_string:len() do
            ...
            if (usermodes[mode]) then
				-- this is a usermode
				chan:_change_status(target, dir == "+", mode)
			end
			callback(({["+"] = "mode", ["-"] = "demode"})[dir],
			         chan, from, target, mode)
			ind = ind + 1
        end
        -- }}}
    elseif from == to then
...
function handlers.on_topic(from, chan, new_topic)
    base.assert(serverinfo.channels[chan],
	 ...
	 local old_topic = serverinfo.channels[chan]._topic.text;
	 serverinfo.channels[chan]._topic.old_topic = old_topic;
    ...
    if serverinfo.channels[chan].join_complete then
        callback("topic_change", serverinfo.channels[chan], serverinfo.channels[chan]._topic)
    end
end
-- }}}
...
function handlers.on_notice(from, to, msg)
	-- stupid gamesurge fix
	if (from == "Global") and not (serverinfo.connected) then
        -- TODO: Get MOTD here too.
...
function handlers.on_ping(from, respond_to)
	 logcat(Sakuya.Logcat.RawIRC, "PING from "..respond_to..".");
...
function handlers.on_rpl_namreply(from, chanmode, chan, userlist)
    ...
    local users = misc._split(userlist)
	 logcat(Sakuya.Logcat.RawIRC, "Parsing NAMES..");
    for k,v in base.ipairs(users) do
		-- collect all users modes and insert into our new
		-- user mode table
		local modes = {};
		
		-- used as associative arrays can't be counted using table.getn or #array
		local has_modes, nick_pos = false, 0;
		
		for i = 1, v:len() do
			if v:sub(i, i):upper() == v:sub(i, i):lower() then
				if (usermodes_symbols[v:sub(i, i)]) then
					has_modes = true;
					modes[v:sub(i, i)] = true;
				else
					nick_pos = i;
					break;
				end
        ...
        if has_modes then
			serverinfo.channels[chan]:_add_user(v:sub(nick_pos), modes)
			logcat(Sakuya.Logcat.RawIRC, v:sub(nick_pos).." has "..v:sub(0, nick_pos-1)..".");
        else
			serverinfo.channels[chan]:_add_user(v)
			logcat(Sakuya.Logcat.RawIRC, v.." has no special privileges.");
        end
    end
	logcat(Sakuya.Logcat.RawIRC, "NAMES parsing complete.");
end
-- }}}
...
function ctcp_handlers.on_dcc(from, to, message)
    ...
    if type == "SEND" then
        if callback("dcc_send", from, to, argument, address, port, size) then
            if (base.pcall(dcc._accept, argument, address, port)) then
				callback("dcc_send_success", from);
			else
				callback("dcc_send_failure", from);
			end
        end
...
function ctcp_handlers.on_version(from, to)
	local version = callback("ctcp", "VERSION", from);
	
	if (version) then
		notice(from, c("VERSION", version));
	else
		notice(from, c("VERSION", VERSION));
	end
end
...
function ctcp_handlers.on_errmsg(from, to, message)
	callback("ctcp", "ERRMSG", from, message);
...
function ctcp_handlers.on_time(from, to)
	local thetime = callback("ctcp", "TIME", from);
	
	if (thetime) then
		notice(from, c("VERSION", thetime));
	else
...
-- on_finger {{{
function ctcp_handlers.on_finger(from, to)
	local finger = callback("ctcp", "FINGER", from);
	
	if (finger) then
		notice(from, c("FINGER", finger))
	else
		notice(from, c("FINGER", FINGERMSG))
	end
end
-- }}}
-- }}}
...
function connect(args)
	...
	if args.nick then NICK = args.nick end
	...
	logcat(Sakuya.Logcat.RawIRC, "Authenticating to server as "..nick.."/"..username.." ("..realname..")..");
    send("NICK", nick)
...
function part(channel, message)
    if not channel then return end
    serverinfo.channels[channel] = nil
    send("PART", channel, message or Sakuya.Configuration.PartMessage or "Leaving")
end
-- }}}
