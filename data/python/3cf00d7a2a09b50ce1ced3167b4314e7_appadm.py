...
class HgADM(object):
	"""lets you mess about with Mercurial commands"""
	def findRepo(self,path=None):
		"""find Repositories within given path, defaults to current working
		dir"""
		ret = None
		if path == None:
			path = os.getcwd()
		...
		if os.path.exists(os.path.join(path,'.hg')):
			...
			for x in os.path.split(str(path)):
				path = x[0]
				print 'new path:', path,x
				if os.path.exists(os.path.join(path,'.hg')):
					#found
					ret = path
		return ret
