...
{
		// EDIT: Steven Wirsz - 12/20/10
		// Modify missile and laser class from 6 to 9 for Mission #52
...
// ADDED: Robert Caldwell - 10/15/10
// return the percent amount of armor strength remaining
// Example: ARMOR1_STRENGTH=120, armorIntegrity=120
// armorIntegrity * 100 = 12000, 12000 / ARMOR1_STRENGTH = 100(%)
float Ship::getArmorDisplayPercentage()
{
	return (armorIntegrity * 100.0f) / getMaxArmorIntegrity();
}
...
// EDIT: Steven Wirsz - 12/20/10
// Modify Shield class from 6 to 8 for Nyssian special case
...
void Ship::ConsumeFuel(int iterations)
{
    for (int n=0; n<iterations; n++)
    {
		...
		// calc fuel consumption, if engine class is less than 2, add 1 to increase fuel
		// efficiency a bit, otherwise add nothing
		float percent_amount = 0.001f / ((engine_class < 2) ? (engine_class + 1) : (engine_class));
...
void Ship::SendDistressSignal()
{
    ...
    g_game->ShowMessageBoxWindow(message, 500, 300, BLUE, 1024/2, 768/2, true, false);
...
void GameState::Reset()
{
	...
	// EDIT: Steven Wirsz - 12/20/10
	// Tweak initial attitudes to better represent the initial communication postures
